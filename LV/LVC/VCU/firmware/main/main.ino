...
bool in_range(float VALUE, float MAX_VALUE, float MIN_VALUE) {
  if ((VALUE > MIN_VLAUE) && (VALUE < MAX_VALUE)) {
    return true;
  } else {
    return false;
  }
...
void check_state() {
  // run necessary checks to determine state
...
void loop () {
  ...
  // check if current and throttle signals hav enot gone out of bounds
  ...
  int throttle_tolterance = MAXTHROTTLEVOLT*0.05;
  if !(((APPS_2_VOLTS_in <= (APPS_1_VOLTS_in + throttle_tolterance)) && (APPS_2_VOLTS_in >= (APPS_1_VOLTS_in - throttle_tolterance)))) {
